...
if __name__ == '__main__':
    ...
    piece_names = []
    #piece_pose = Pose(position=Point(x=0.5, y=0.6, z=0.8))
    piece_pose = deepcopy(board_pose)
    piece_pose.position.x = 0.6
    piece_pose.position.y = 0.6
    piece_pose.position.z = 0.8
    ...
    for row, each in enumerate(board_setup):
        ...
        for col, piece in enumerate(each):
            ...
            if piece in list_pieces:
                try:
                    spawn_sdf = rospy.ServiceProxy('/gazebo/spawn_sdf_model', SpawnModel)
                    spawn_sdf("%s%d" % (piece,col), pieces_xml[piece], "/", piece_pose, "world")
                except rospy.ServiceException, e:
                    rospy.logerr("Spawn SDF service call failed: {0}".format(e))

                piece_names.append("%s%d" % (piece,col))

                print ("picking %s%d" % (piece,col))
                pnp.pick(Pose(position=Point(x=piece_pose.position.x, y=piece_pose.position.y, z=-0.14), orientation=overhead_orientation))
                print ("placing %s%d" % (piece,col))
